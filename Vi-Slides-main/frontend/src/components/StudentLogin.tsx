...
function StudentLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Submitting student login with:', payload);
    ...
    try {
      ...
      if (data.user.role !== 'student') {
        setError('This account is a teacher account. Please use teacher login.');
        setLoading(false);
        return;
      }
      ...
      console.log('Student login successful, redirecting to /student');
      setLoading(false);
      navigate('/student');
    } catch (err) {
      ...
      <div className="auth-card">
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          ...
          <button type="submit" className="auth-submit" disabled={loading}>
            {loading ? <span className="auth-spinner" /> : 'Sign in as Student'}
          </button>
        </form>
        <p className="auth-switch">
          Don't have a student account? <Link to="/signup?role=student" className="auth-link">Create one →</Link>
        </p>
        <p className="auth-switch">
          Are you a teacher? <Link to="/teacher-login" className="auth-link">Teacher Login →</Link>
        </p>
      </div>
