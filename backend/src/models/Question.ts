...
export interface IQuestion extends Document {
    content: string;
    question: string; // For RAG system - same as content
    answer: string; // RAG system answer
    status: 'active' | 'archived' | 'auto_answered' | 'unanswered'; // Added RAG statuses
    sessionId: string; // For RAG system queries
    timestamp: Date; // For RAG system queries
    user?: mongoose.Types.ObjectId; // Optional for guest questions
    guestName?: string; // For non-authenticated questions
    guestEmail?: string; // For non-authenticated questions
    session: mongoose.Types.ObjectId;
...
const questionSchema = new Schema<IQuestion>({
    ...
    question: {
        type: String,
        required: [true, 'Question is required for RAG system'],
        trim: true,
        maxlength: [500, 'Question cannot be more than 500 characters']
    },
    answer: {
    ...
    sessionId: {
        type: String,
        required: [true, 'Session ID is required for RAG system'],
        trim: true
    },
    timestamp: {
        type: Date,
    ...
    status: {
        type: String,
        enum: ['active', 'archived', 'auto_answered', 'unanswered'],
        default: 'active'
    },
    analysisStatus: {
        type: String,
